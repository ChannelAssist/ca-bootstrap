...
- `ca-bootstrap` will never delete a non-empty folder, or any folder containing sub-folders, without explicit user confirmation. Sub-folders may belong to other tools (Claude Cowork, IDE scratch); the rule is enforced across `steps/50-folders.ps1`, `commands/repair.ps1`, and `commands/undo.ps1` (AB#40007).
