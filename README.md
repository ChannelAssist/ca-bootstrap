...
5. **Folder structure** — creates the workspace taxonomy (`ca-tools-repo/`, `ca-docs-repo/`, `ca-platform-repo/`, `cm-product-repo/`, `ca-training-repo/`, `ca-work-dirs-repo/`), migrates renamed predecessors (e.g. an existing `ca-tools/` is renamed to `ca-tools-repo/`), and seeds a per-folder `README.md`.
