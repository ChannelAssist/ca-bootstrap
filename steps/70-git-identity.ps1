...
function Test-CABStep70 {
    ...
    if ($content -like "*$needle*") {
        # Also verify the per-workspace .gitconfig the includeIf points
        # at actually exists on disk. After undo or a manual `rm`, the
        # includeIf can be left dangling — git would silently fall back
        # to the global identity for ChannelAssist commits. Doctor flags
...
function Invoke-CABStep70 {
    ...
    # .gitconfig file is gone. We have the previous name/email recorded
    ...
    if ($existingIncludeIf) {
        ...
        $priorEmail = if ($prior) { [string]$prior.new_workspace_email } else { '' }
        if ($priorName -and $priorEmail) {
            Write-CABColor DarkGray "  Restoring workspace .gitconfig from journal: $priorName <$priorEmail>"
            $workspaceContent = @"
...
"@
            ...
            return @{ status = 'ok'; details = "Restored workspace .gitconfig with prior identity: $priorName <$priorEmail>" }
        }
        # Fall through: no usable journal data, ask the user.
        Write-CABColor Yellow "  No prior identity in the journal — re-prompting."
    }
