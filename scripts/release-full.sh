...
cleanup_on_exit() {
    ...
    if [ "$RULESET_DISABLED" = '1' ] \
        ...
        warn "exit $rc while dev-protection disabled — restoring..."
        if jq -M -c '.enforcement = "active" | {name, target, source_type, source, enforcement, conditions, rules}' \
...
Auto-generated by make release-full VERSION=$VERSION.
...
# DRY_RUN short-circuit: under dry-run we didn't actually bump dev, so
# release.sh's `version constant on origin/dev matches VERSION` check
# would fail with a misleading error. Instead, summarise what the next
# step would do and exit 0 so the dry-run reports success of the
# overall plan rather than tripping on a precondition we never set up.
if [ -n "$DRY_RUN" ]; then
    ...
    info '         unchanged version constant). Run scripts/release.sh'
    info "         directly with DRY_RUN=1 if you want to dry-run that"
    info '         half in isolation.'
    ok "DRY_RUN: release-full plan validated end-to-end (no mutations)."
    exit 0
fi
